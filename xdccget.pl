...
$debug=0;
my $config_path = Irssi::settings_get_str('xdccget_config_path') || "$ENV{HOME}/.irssi";
my $max_downloads = Irssi::settings_get_int('xdccget_max_downloads') || 2;
my $queue_file = "$config_path/xdccget.queue";
sub saveQueue {
	if (!open (QUEUE, ">$queue_file")) {
...
sub loadQueue {
	@queue = ();
	if (!open (QUEUE, "<$queue_file")) {
		print CLIENTCRAP "XDCCGET - ERROR! Cannot open queue file for loading from $queue_file: $!";
		return;
	}
	while (<QUEUE>) {
		chomp;
...
sub process_queue {
	...
	for ($i=0; $i<= $#queue; $i++) {
		...
		if ($queue[$i]{'status'} eq 'completed' ||
			...
			open (DONEFILE, ">>$config_path/xdccdone.txt");
...
Irssi::settings_add_int($IRSSI{'name'}, 'xdccget_retry_time', 60);
